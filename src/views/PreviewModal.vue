      ...
      <v-card>
		...
		<v-card tile v-if="imageUrl !== ''" class="mb-12" id="image-card">
			  <v-img height="50vh"  :src="imageUrl">
				  <v-flex id="title-preview-margin" class="mx-auto align-center">
					<h1 style="font-size:40px;" v-html="itemList[0].title"></h1>
					<div style="opacity:80%;"><h3 v-html="itemList[0].subtitle"></h3></div>
					<div style="opacity:60%;margin-top:30px;"><h5>{{nickName}} · {{date}}</h5></div>
				  </v-flex>
			  </v-img>
		</v-card>

		<!--Title content-->
		<v-flex v-if="imageUrl == ''" id="title-content-preview-margin" class="mx-auto mb-12 align-center">
			<h1 style="font-size:40px;" v-html="itemList[0].title"></h1>
			<div style="opacity:80%;"><h3 v-html="itemList[0].subtitle"></h3></div>
			<div style="opacity:60%;margin-top:30px;"><h5>{{nickName}} · {{date}}</h5></div>
		</v-flex>
		...
		<!-- Content -->
		<v-flex id="content-preview-margin" class="mx-auto align-center">
			<div>
		...
		<!-- Tags -->
		<v-flex id="tag-preview-margin" class="mx-auto mt-8 pb-8 align-center">
...
<script lang="ts">
	...
	export default class PreviewModal extends Vue {
		...
		@WriteStoreModule.State('nickName')
